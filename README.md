...
| 四.3 方向性保护 | `strategy/protection.rs`、`market/impact.rs` | v₁₀₀ ≥ 1δ 轻度 / ≥ 2δ 强；Gate 100 ms 单边占比 > 80% 且成交额 > 95% 分位；轻度：半量+1.5 倍距离；强：撤该侧开仓单、另一侧半量紧跟；恢复 500 ms 半量 → 2 s 全量；减仓单不受保护缩量影响，强信号逆向持仓 → 立即主动减仓 |
...
| 五.3 库存退出 | `strategy/inventory.rs`、`engine/mod.rs` | 带外 > 3 s → 停止加仓并主动减回 ±0.1H；计时只在回带且无在途加仓单时结束；强逆向信号不等 3 s；达 H 或亏损限额 → 清仓并停止 |
...
- **强逆向信号触发的主动减仓**目标为清掉逆向净头寸（不是减到目标带），与方案 §4.3 "已有空仓就不能继续等 maker 买单" 一致。
